...
require_match "$CI_WORKFLOW" '\./get-tak\.sh' "CI Tak bootstrap command"
require_match "$CI_WORKFLOW" 'TAK_INSTALL_DIR:\s*\$\{\{ runner\.temp \}\}/tak-bin' "CI Tak install dir"
...
require_match "$CI_WORKFLOW" 'tak run //:check' "CI Tak check command"
require_match "$CI_WORKFLOW" 'tak run //:coverage' "CI Tak coverage command"
...
require_match "$RELEASE_WORKFLOW" '\./get-tak\.sh' "Release Tak bootstrap command"
require_match "$RELEASE_WORKFLOW" 'TAK_INSTALL_DIR:\s*\$\{\{ runner\.temp \}\}/tak-bin' "Release Tak install dir"
