...
require_match "$CI_WORKFLOW" 'cargo run --locked -p tak -- run //:check' "CI Tak check command"
require_match "$CI_WORKFLOW" 'cargo run --locked -p tak -- run //:coverage' "CI Tak coverage command"
require_match "$CI_WORKFLOW" 'cargo run --locked -p tak -- run "\$\{\{ matrix\.tak_task \}\}"' "CI Tak matrix build command"
...
require_match "$RELEASE_WORKFLOW" 'cargo run --locked -p tak -- run "\$\{\{ matrix\.tak_task \}\}"' "Release Tak package command"
