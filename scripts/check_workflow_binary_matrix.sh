...
require_match "$CI_WORKFLOW" 'path: \.tmp/coverage/lcov\.info\s*\n\s*include-hidden-files: true\s*\n\s*if-no-files-found: error' "CI required hidden coverage artifact upload"
...
require_match "$CI_WORKFLOW" 'RUSTUP_HOME="\$\(rustup show home\)" tak run //:ci --local-no-container --pass-env CARGO_HOME --pass-env RUSTUP_HOME' "CI host-only Tak command with explicit Rust homes"
require_match "$CI_WORKFLOW" 'RUSTUP_HOME="\$\(rustup show home\)" tak run "\$\{\{ matrix\.tak_task \}\}" --pass-env CARGO_HOME --pass-env RUSTUP_HOME' "CI Tak matrix build command with explicit Rust homes"
require_match "$CI_WORKFLOW" 'uses:\s*mlugg/setup-zig@v2' "CI maintained Zig setup action"
require_no_match "$CI_WORKFLOW" 'uses:\s*goto-bus-stop/setup-zig@v2' "CI legacy Zig setup action"
...
require_match "$RELEASE_WORKFLOW" "workflow_run:" "Release successful-CI workflow_run trigger"
require_match "$RELEASE_WORKFLOW" 'workflows:\s*\n\s*-\s*CI' "Release watches CI workflow"
require_match "$RELEASE_WORKFLOW" 'types:\s*\n\s*-\s*completed' "Release waits for completed CI runs"
require_no_match "$RELEASE_WORKFLOW" '^  push:' "Release direct push trigger"
require_match "$RELEASE_WORKFLOW" 'cancel-in-progress:\s*false' "Release queue preserves successful main builds"
...
require_match "$RELEASE_WORKFLOW" "github.event.workflow_run.conclusion == 'success'" "Release successful CI condition"
require_match "$RELEASE_WORKFLOW" "github.event.workflow_run.event == 'push'" "Release push-only workflow_run condition"
require_match "$RELEASE_WORKFLOW" "github.event.workflow_run.head_branch == 'main'" "Release main-only condition"
require_match "$RELEASE_WORKFLOW" 'scripts/compute_release_version\.sh' "Release incremental version script"
...
require_match "$RELEASE_WORKFLOW" 'RUSTUP_HOME="\$\(rustup show home\)" tak run "\$\{\{ matrix\.tak_task \}\}"' "Release Tak package command"
require_match "$RELEASE_WORKFLOW" 'tak run "\$\{\{ matrix\.tak_task \}\}" --pass-env CARGO_HOME --pass-env RUSTUP_HOME --pass-env TAK_RELEASE_TAG --pass-env TAK_BUILD_VERSION' "Release Tak package environment allowlist"
...
require_no_match "$RELEASE_WORKFLOW" '--prerelease' "Release non-main prerelease publishing"
