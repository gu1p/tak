...
require_match "$RELEASE_WORKFLOW" "workflow_run\\.conclusion == 'success'" "Release success gate"
require_match "$RELEASE_WORKFLOW" "workflow_run\\.event == 'push'" "Release push-event gate"
require_match "$RELEASE_WORKFLOW" "workflow_run\\.head_branch != ''" "Release branch-only gate"
