...
/// Detects the workspace root based on `taskcraft.toml`, `.git`, or the current path.
///
/// ```no_run
/// # // Reason: This behavior depends on internal state and is compile-checked only.
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
pub fn detect_workspace_root(start: &Path) -> Result<PathBuf> {
    ...
    if let Some(marker) = find_ancestor_with(&search_start, "taskcraft.toml") {
        return Ok(marker);
    }
...
pub fn load_workspace(root: &Path, options: &LoadOptions) -> Result<WorkspaceSpec> {
    ...
    let dep_map: BTreeMap<TaskLabel, Vec<TaskLabel>> = tasks
        ...
        .collect();
    taskcraft_core::planner::topo_sort(&dep_map).context("invalid task graph")?;
...
struct TaskcraftToml {
    project_id: Option<String>,
}

/// Resolves the project id from options, `taskcraft.toml`, or a path-based hash fallback.
///
/// ```no_run
/// # // Reason: This behavior depends on internal state and is compile-checked only.
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
fn resolve_project_id(root: &Path, from_options: Option<&str>) -> Result<String> {
    ...
    let config_file = root.join("taskcraft.toml");
    if config_file.exists() {
        let config_data = fs::read_to_string(&config_file)?;
        let config: TaskcraftToml = toml::from_str(&config_data)
...
/// Converts a discovered `TASKS.py` path into a Taskcraft package label prefix.
///
/// ```no_run
/// # // Reason: This behavior depends on internal state and is compile-checked only.
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
