...
use taskcraft_core::model::{BackoffDef, NeedDef, ResolvedTask, StepDef, TaskLabel, WorkspaceSpec};
use taskcraftd::{
...
pub async fn run_tasks(
    ...
    let order =
        taskcraft_core::planner::topo_sort(&dep_map).context("failed to order task execution")?;
...
impl LeaseContext {
    ...
    fn from_options(options: &RunOptions) -> Self {
        ...
        let session_id = options
            .session_id
            .clone()
            .unwrap_or_else(|| format!("taskcraft-{}", Uuid::new_v4()));
