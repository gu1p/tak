...
lint:
	@touch lint.started
	@for i in $$(seq 1 100); do test -f test.started -a -f build.started && break; sleep 0.02; done
	@test -f test.started -a -f build.started
	@touch lint.done
test:
	@touch test.started
	@for i in $$(seq 1 100); do test -f lint.started -a -f build.started && break; sleep 0.02; done
	@test -f lint.started -a -f build.started
	@touch test.done
build:
	@touch build.started
	@for i in $$(seq 1 100); do test -f lint.started -a -f test.started && break; sleep 0.02; done
