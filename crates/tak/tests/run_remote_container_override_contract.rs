...
fn run_command_uses_tasks_default_container_runtime_with_remote_flag() -> Result<()> {
    ...
    write_tasks(
        &workspace_root,
        r#"SPEC = module_spec(defaults=Defaults(container_runtime=Runtime.Image("alpine:3.20")), tasks=[task("check", outputs=[path("out")], steps=[cmd("sh", "-c", "mkdir -p out && printf '%s\n' \"$TAK_RUNTIME_SOURCE\" > out/runtime-source.txt")])])
SPEC
"#,
    )?;
