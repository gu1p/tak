...
fn run_command_executes_neutral_task_remotely_with_remote_flag() -> Result<()> {
    ...
    write_tasks(
        &workspace_root,
        "SPEC = module_spec(defaults=Defaults(container_runtime=Runtime.Image(\"alpine:3.20\")), tasks=[task(\"check\", outputs=[path(\"out\")], steps=[cmd(\"sh\", \"-c\", \"mkdir -p out && echo remote-run > out/remote-run.txt\")])])\nSPEC\n",
    )?;
