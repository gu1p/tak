//! Optional black-box E2E contract for containerized tak client + Tor remote container runtime.
...
use support::e2e_harness::{
    detect_podman_socket, find_free_tcp_port, run_tak_in_container_expect_success,
    spawn_daemon_with_remote_bind, write_tasks,
};

#[test]
fn e2e_containerized_tak_client_runs_remote_tor_container_and_returns_outputs() -> Result<()> {
    if std::env::var("TAK_E2E_REAL_CONTAINER").ok().as_deref() != Some("1") {
        eprintln!("skipping: set TAK_E2E_REAL_CONTAINER=1 to run containerized tak E2E contract");
        return Ok(());
    }
...
"#
    ...
    let _remote_worker = spawn_daemon_with_remote_bind(
        temp.path().join("remote-worker.sock"),
        temp.path().join("remote-worker.sqlite"),
        Some(format!("0.0.0.0:{remote_port}")),
        &env,
    )?;

    let run = run_tak_in_container_expect_success(
        temp.path(),
