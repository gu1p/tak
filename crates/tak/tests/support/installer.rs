...
fn fake_curl() -> &'static str {
    "#!/usr/bin/env bash\nif [[ \"$*\" == *\"releases/latest\"* ]]; then echo '{\"tag_name\":\"v1.0.0\"}'; exit 0; fi\nout=''; prev=''; for arg in \"$@\"; do if [[ \"$prev\" == '-o' ]]; then out=\"$arg\"; fi; prev=\"$arg\"; done\nmkdir -p \"$(dirname \"$out\")/pkg\"; cat >\"$(dirname \"$out\")/pkg/takd\" <<'EOF'\n#!/usr/bin/env bash\nset -euo pipefail\nstate_root=\"${XDG_STATE_HOME:-$HOME/.local/state}/takd\"\nconfig_root=\"${XDG_CONFIG_HOME:-$HOME/.config}/takd\"\nmkdir -p \"$state_root\" \"$config_root\"\nrecord_pending() {\n  printf 'transport_state = \"pending\"\\nbase_url = \"http://test-hidden-service.onion\"\\ndetail = \"self-probe connect attempt 3 failed: rendezvous circuit timed out\"\\n' >\"$state_root/transport-health.toml\"\n  printf 'bootstrapping embedded Arti\\nself-probe connect attempt 3 failed: rendezvous circuit timed out\\n' >>\"$state_root/service.log\"\n}\ncase \"$1 ${2:-}\" in\n  'init '*)\n    if [[ ! -f \"$config_root/agent.toml\" ]]; then\n      printf 'display_name = \"%s\"\\n' \"${TAKD_DISPLAY_NAME:-fake-agent}\" >\"$config_root/agent.toml\"\n    fi\n    ;;\n  'token show'*)\n    attempt_file=\"$state_root/token-attempts\"\n    attempt=0; [[ -f \"$attempt_file\" ]] && attempt=\"$(cat \"$attempt_file\")\"\n    attempt=$((attempt + 1)); printf '%s' \"$attempt\" >\"$attempt_file\"\n    if [[ \"${TAKD_INSTALLER_FAKE_TOKEN_FAIL:-}\" == '1' || \"$attempt\" -le \"${TAKD_INSTALLER_FAKE_PENDING_ATTEMPTS:-0}\" ]]; then\n      record_pending\n      printf 'Error: tor transport is pending for http://test-hidden-service.onion: self-probe connect attempt 3 failed: rendezvous circuit timed out\\n' >&2\n      exit 1\n    fi\n    if [[ \"$*\" == *\"--qr\"* ]]; then\n      printf \"Scan this QR code\\n████████\\n██    ██\\n██ ██ ██\\n████████\\ntak remote add 'takd:tor:test-hidden-service.onion:ABCDE'\\ntakd:tor:test-hidden-service.onion:ABCDE\\n\"\n    else\n      echo 'takd:tor:test-hidden-service.onion:ABCDE'\n    fi\n    ;;\n  'logs '*) printf 'bootstrapping embedded Arti\\nself-probe connect attempt 3 failed: rendezvous circuit timed out\\n' ;;\n  'status '*) printf 'node_id: fake-agent\\ntransport: tor\\ntransport_state: pending\\nbase_url: http://test-hidden-service.onion\\ntransport_detail: self-probe connect attempt 3 failed: rendezvous circuit timed out\\nlog_path: %s/service.log\\n' \"$state_root\" ;;\n  'serve '*) : ;;\nesac\nEOF\nchmod +x \"$(dirname \"$out\")/pkg/takd\"\ntar -czf \"$out\" -C \"$(dirname \"$out\")/pkg\" takd\n"
}
