...
fn run_remote_syncs_declared_output_globs_only() -> Result<()> {
  ...
  execution=Execution.Remote(pool="build", required_tags=["builder"], required_capabilities=["linux"], transport=Transport.DirectHttps(), container=Container.Image("alpine:3.20")),
  steps=[cmd("sh", "-ceu", """
