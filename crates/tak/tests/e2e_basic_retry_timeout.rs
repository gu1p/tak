...
#[allow(dead_code)]
mod support;
use support::e2e_harness::{run_tak_expect_failure, run_tak_expect_success, write_tasks};
...
"#,
    ...
    let run_retry =
        run_tak_expect_success(temp.path(), &["run", "apps/web:retry_task"], None, &env)?;
    assert!(run_retry.contains("apps/web:retry_task: ok (attempts=2"));
    assert_eq!(fs::read_to_string(&retry_out)?.trim(), "recovered");

    let (_stdout, stderr) =
        run_tak_expect_failure(temp.path(), &["run", "apps/web:timeout_task"], None, &env)?;
    assert!(
