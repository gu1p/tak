...
fn run_command_uses_tasks_default_container_runtime_with_local_container_flags() -> Result<()> {
    ...
    write_tasks(
        temp.path(),
        r#"SPEC = module_spec(defaults=Defaults(container=Container.Dockerfile(path("docker/Dockerfile"), resources=Container.Resources(cpu_cores=1.0, memory_mb=512))), tasks=[task("check", steps=[cmd("sh", "-c", "mkdir -p out && printf '%s\n' \"$TAK_RUNTIME_SOURCE\" > out/runtime-source.txt")])])
SPEC
"#,
    )?;
