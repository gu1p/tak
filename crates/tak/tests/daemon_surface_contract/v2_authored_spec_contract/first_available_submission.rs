...
#[test]
fn first_available_preserves_balanced_remote_tier_before_local_fallback() {
    ...
    let job = &requests[1]["operation"]["run"]["jobs"][0];
    ...
    assert_eq!(
        ...
        json!([
            ...
            {"node_id": "local", "kind": "local", "transport": null,
             "reason": "local execution", "tier": 1}
        ])
    );
}

#[test]
fn first_available_submits_local_fallback_when_remote_tier_is_empty() {
    std::fs::create_dir_all(".tmp").unwrap();
    let temp = tempfile::tempdir_in(".tmp").unwrap();
    let workspace = temp.path().join("workspace");
    let socket = PathBuf::from(".tmp")
    ...
    let requests = daemon.finish_expecting(6);
    let candidates = &requests[1]["operation"]["run"]["jobs"][0]["placement_candidates"];
    assert_eq!(
        candidates,
        &json!([{"node_id": "local", "kind": "local", "transport": null,
                 "reason": "local execution", "tier": 1}])
    );
}

const TASKS: &str = r#"POLICY = Execution.FirstAvailable(placements=[
  Execution.Remote(required_tags=["builder"], required_capabilities=["linux"],
