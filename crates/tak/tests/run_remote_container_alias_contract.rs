...
fn run_command_warns_that_remote_container_flag_is_redundant() -> Result<()> {
    ...
    write_tasks(
        &workspace_root,
        r#"SPEC = module_spec(defaults=Defaults(container=Container.Dockerfile(path("docker/Dockerfile"), resources=Container.Resources(cpu_cores=1.0, memory_mb=512))), tasks=[task("check", outputs=[path("out")], steps=[cmd("sh", "-c", "mkdir -p out && printf '%s\n' \"$TAK_RUNTIME_SOURCE\" > out/runtime-source.txt")])])
SPEC
"#,
    )?;
