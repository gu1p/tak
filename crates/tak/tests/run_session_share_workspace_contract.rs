...
SPEC = module_spec(
  tasks=[
    task("build", steps=[cmd("sh", "-c", "mkdir -p .session && echo cached > .session/build.txt")], use_session=SESSION),
    task("test", deps=[":build"], outputs=[path("out")], steps=[cmd("sh", "-c", "test -f .session/build.txt && mkdir -p out && cat .session/build.txt > out/result.txt")], use_session=SESSION),
  ],
)
SPEC
"#,
...
SPEC = module_spec(
  tasks=[
    task("build", steps=[cmd("sh", "-c", "mkdir -p .session && echo remote-cached > .session/build.txt")], use_session=SESSION),
    task("test", deps=[":build"], outputs=[path("out")], steps=[cmd("sh", "-c", "test -f .session/build.txt && mkdir -p out && cat .session/build.txt > out/remote-result.txt")], use_session=SESSION),
  ],
)
SPEC
"#,
