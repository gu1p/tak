...
#[allow(dead_code)]
mod support;
use support::e2e_harness::{run_tak_expect_success, write_tasks};
...
"#,
    ...
    let list = run_tak_expect_success(temp.path(), &["list"], None, &env)?;
    assert!(list.contains("//apps/web:build"));
    assert!(list.contains("//apps/web:test"));

    let explain = run_tak_expect_success(temp.path(), &["explain", "apps/web:test"], None, &env)?;
    ...
    let tree = run_tak_expect_success(temp.path(), &["tree"], None, &env)?;
    assert!(tree.contains("Tak Tree"));
    assert!(tree.contains("//apps/web:test"));

    let run = run_tak_expect_success(temp.path(), &["run", "apps/web:test"], None, &env)?;
