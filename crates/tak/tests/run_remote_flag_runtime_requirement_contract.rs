...
"#,
    ...
    assert!(
        stderr.contains(
            "task //:check requires a container for --remote; provide --container-image, --container-dockerfile, Execution.Remote(..., container=Container.Image(...)), or TASKS.py defaults.container"
        ),
        "stderr:\n{stderr}"
    );
