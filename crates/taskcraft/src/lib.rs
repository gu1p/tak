//! Taskcraft command-line interface library.
//!
//! This crate exposes the CLI runtime used by the `taskcraft` binary. Moving behavior
...
use taskcraft_core::model::{TaskLabel, WorkspaceSpec};
use taskcraft_exec::{RunOptions, run_tasks};
use taskcraft_loader::{LoadOptions, load_workspace};
use taskcraftd::{Request, Response, StatusRequest};
...
#[command(about = "Taskcraft task orchestrator")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
/// Parses CLI input and dispatches Taskcraft commands.
///
/// ```no_run
        ...
        Commands::Run {
            ...
            let summary = run_tasks(
                ...
                &RunOptions {
                    ...
                    lease_ttl_ms: env_u64("TASKCRAFT_LEASE_TTL_MS", 30_000),
                    lease_poll_interval_ms: env_u64("TASKCRAFT_LEASE_POLL_MS", 200),
                    session_id: std::env::var("TASKCRAFT_SESSION_ID").ok(),
                    user: std::env::var("TASKCRAFT_USER").ok(),
                },
            )
            .await?;
...
fn resolve_daemon_socket_path() -> PathBuf {
    std::env::var("TASKCRAFTD_SOCKET")
        .map(PathBuf::from)
        .unwrap_or_else(|_| taskcraftd::default_socket_path())
}
...
/// # }
/// ```
async fn query_daemon_status(socket_path: PathBuf) -> Result<taskcraftd::StatusSnapshot> {
