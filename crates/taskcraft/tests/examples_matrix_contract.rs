...
fn maybe_start_daemon(case: &ExampleCase, temp_root: &Path) -> Option<RunningDaemon> {
    ...
    let socket = temp_root.join("taskcraftd.sock");
    let db_path = temp_root.join("taskcraftd.sqlite");
...
/// Runs one `taskcraft` command for an example and asserts expected outcome.
fn run_taskcraft_command(
    working_dir: &Path,
    daemon_socket: Option<&Path>,
    args: &[&str],
    expect_success: bool,
) {
    let mut command = StdCommand::new(assert_cmd::cargo::cargo_bin!("taskcraft"));
    command.current_dir(working_dir).args(args);
    if let Some(socket) = daemon_socket {
        command.env("TASKCRAFTD_SOCKET", socket);
    }

    let output = command.output().unwrap_or_else(|err| {
        panic!(
            "failed executing taskcraft {:?} in {}: {err}",
            args,
            working_dir.display()
        )
    });
