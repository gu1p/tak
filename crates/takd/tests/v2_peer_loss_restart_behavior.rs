...
async fn first_probe_failure_resolves_persisted_attempts_without_poisoning_idle_workers() {
    ...
    drop(unavailable);
    let peers = v2_node_loss::restarted_peer_manager(&endpoint);
    ...
    .await
    .expect("restart loss did not resolve durable attempts");
