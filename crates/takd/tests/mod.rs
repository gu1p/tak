mod agent_cli_contract;
mod agent_log_tail_bounded_contract;
mod agent_logs_all_contract;
mod agent_logs_contract;
mod agent_no_diagnose_contract;
mod agent_service_logging_contract;
mod agent_service_logging_failure_contract;
mod agent_service_logging_source_contract;
mod agent_status_advertised_contract;
mod agent_status_contract;
mod agent_status_pending_advertised_contract;
mod agent_status_recovering_contract;
mod cli_binary_contract;
mod container_behavior;
mod daemon_cli_contract;
mod daemon_protocol_lifecycle_behavior;
mod daemon_protocol_request_behavior;
mod daemon_socket_permissions_contract;
mod direct_agent_base_url_contract;
mod direct_agent_base_url_normalization_contract;
mod direct_agent_onboarding_contract;
mod direct_agent_uppercase_base_url_recovery_contract;
mod idempotency_behavior;
mod lease_behavior;
mod lease_persistence_failure_behavior;
mod live_tor_http_timeout_contract;
mod local_protocol_v2_admission_contract;
mod local_protocol_v2_admission_integration;
mod local_protocol_v2_log_secrecy_contract;
mod local_protocol_v2_run_persistence_integration;
mod local_tor_broker_contract;
mod local_tor_broker_http2_session_contract;
mod local_tor_broker_regression_contract;
mod no_runtime_expect_contract;
mod remote_contracts;
mod remote_exec_root_explicit_behavior;
mod remote_exec_root_fallback_behavior;
mod remote_exec_root_probe_arm64_behavior;
mod remote_exec_root_probe_behavior;
mod remote_exec_root_probe_retry_behavior;
mod remote_exec_root_probe_unknown_arch_behavior;
mod remote_exec_root_probe_version_error_behavior;
mod remote_exec_root_simulated_runtime_behavior;
mod remote_image_cache_janitor_behavior;
mod remote_node_log_tail_bounded_contract;
mod remote_node_logs_contract;
mod remote_node_transport_state_behavior;
mod remote_orphan_watchdog_behavior;
mod remote_output_cleanup_persistence_behavior;
mod remote_output_cleanup_tolerance_behavior;
mod remote_output_legacy_root_behavior;
mod remote_output_query_decode_behavior;
mod remote_output_query_reserved_character_behavior;
mod remote_output_range_resume_contract;
mod remote_output_root_switch_behavior;
mod remote_resource_admission_behavior;
mod remote_session_contract;
mod remote_session_key_validation_behavior;
mod remote_status_behavior;
mod remote_status_unavailable_behavior;
mod remote_streaming_binary_contract;
mod remote_streaming_chunk_bytes_contract;
mod remote_submit_runtime_required_behavior;
mod remote_task_list_contract;
mod remote_v1_http2_server_contract;
mod remote_v1_http_request_parse_behavior;
mod remote_v1_http_request_validation_behavior;
mod remote_v1_http_server_behavior;
mod remote_v1_http_server_tor_behavior;
mod remote_v1_http_truncated_submit_behavior;
mod remote_worker_timing_behavior;
mod resumable_workspace_upload_contract;
mod runtime_paths_contract;
mod serve_single_instance_contract;
mod serve_tor_readiness_contract;
mod service_direct_behavior;
mod service_tor_behavior;
mod service_tor_live_behavior;
mod service_tor_recovery_behavior;
mod service_tor_startup_recovery_behavior;
mod submit_store_contract;
pub mod support;
mod task_list_contract;
mod task_logs_contract;
mod task_logs_failure_contract;
mod task_logs_follow_contract;
mod task_store_contract;
mod task_store_migration_contract;
mod token_show_qr_contract;
mod token_show_qr_non_v3_contract;
mod token_show_words_contract;
mod token_show_words_table_contract;
mod token_wait_contract;
mod token_wait_malformed_health_contract;
mod token_wait_pending_detail_contract;
mod token_wait_transport_contract;
mod tor_modules;
mod update_contracts;
mod v2_attempt_cancellation_behavior;
mod v2_attempt_coordinator_behavior;
mod v2_attempt_coordinator_failure_contract;
mod v2_attempt_coordinator_missing_contract;
mod v2_attempt_outbox_recovery_behavior;
mod v2_run_store_active_migration_contract;
mod v2_run_store_archive_contract;
mod v2_run_store_blob_recovery_contract;
mod v2_run_store_cancel_migration_contract;
mod v2_run_store_cancellation_contract;
mod v2_run_store_contract;
mod v2_run_store_corrupt_resume_contract;
mod v2_run_store_event_paging_contract;
mod v2_run_store_idempotency_contract;
mod v2_run_store_rate_bucket_migration_contract;
mod v2_run_store_resume_repair_contract;
mod v2_run_store_schema_migration_contract;
mod v2_scheduler_affinity_behavior;
mod v2_scheduler_affinity_group_behavior;
mod v2_scheduler_affinity_migration_conflict_contract;
mod v2_scheduler_affinity_migration_contract;
mod v2_scheduler_atomic_reservation_contract;
mod v2_scheduler_backoff_contract;
mod v2_scheduler_balanced_behavior;
mod v2_scheduler_cancellation_contract;
mod v2_scheduler_constraint_cancellation_behavior;
mod v2_scheduler_definition_conflict_behavior;
mod v2_scheduler_definition_scope_conflict_behavior;
mod v2_scheduler_fairness_behavior;
mod v2_scheduler_fencing_contract;
mod v2_scheduler_limiter_behavior;
mod v2_scheduler_limiter_kinds_behavior;
mod v2_scheduler_node_loss_behavior;
mod v2_scheduler_node_loss_retry_behavior;
mod v2_scheduler_queue_behavior;
mod v2_scheduler_rate_atomic_behavior;
mod v2_scheduler_rate_rollback_contract;
mod v2_scheduler_readiness_contract;
mod v2_scheduler_ready_age_contract;
mod v2_scheduler_retry_contract;
mod v2_scheduler_round_robin_behavior;
mod v2_scheduler_scope_behavior;
mod v2_scheduler_scope_key_behavior;
mod v2_scheduler_shared_workspace_behavior;
mod v2_scheduler_soft_affinity_behavior;
mod v2_scheduler_soft_node_loss_behavior;
mod v2_scheduler_worktree_queue_conflict_behavior;
mod v2_scheduler_worktree_scope_behavior;
mod workspace_wormhole_upload_contract;
