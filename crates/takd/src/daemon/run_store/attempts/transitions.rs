...
use tak_core::v2::{ResolvedJob, RetryPolicy};
...
pub(super) fn schedule_retry(
    ...
    let now = now_ms()?;
    let eligible = now
        .saturating_add(retry_delay(&job.retry, command.authored_attempt))
        .min(i64::MAX as u64);
...
fn retry_delay(retry: &RetryPolicy, authored_attempt: u32) -> u64 {
