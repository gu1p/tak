...
use tak_core::model::{
    CurrentStateSpec, OutputSelectorSpec, ResolvedTask, SessionReuseSpec, SessionUseSpec,
};

use crate::engine::session_tempdir::session_tempdir;
use crate::engine::session_workspace_files::{
    copy_directory_contents, extract_share_paths, materialize_seed_workspace,
};

use super::{ExecutionSessionManager, PreparedTaskSession, SessionWorkspace};
...
impl ExecutionSessionManager {
    ...
    pub(crate) fn prepare_task(
        ...
        let Some(session) = selected_session.or(task.session.as_ref()) else {
            return Ok(None);
        };
        if !use_local_workspace {
            return Ok(Some(self.prepared_session(session, None, None)));
        }
        ...
        match &session.reuse {
            SessionReuseSpec::ShareWorkspace => {
                ...
                Ok(Some(self.prepared_session(session, Some(root), None)))
            }
            SessionReuseSpec::SharePaths { paths } => {
                let prepared = self.share_paths_task(&session.name, paths, seed, workspace_root)?;
                Ok(Some(self.prepared_session(
                    session,
                    Some(prepared.path().to_path_buf()),
                    Some(prepared),
                )))
            }
            SessionReuseSpec::Container => Ok(Some(self.prepared_session(session, None, None))),
        }
    }

    /// Builds a [`PreparedTaskSession`] that copies the session's identity and is
    /// keyed by this run; only the workspace `root` and owned temp dir vary.
    ///
    /// ```no_run
    /// # // Reason: This private builder needs an ExecutionSessionManager and is covered by tests.
    /// # fn main() {}
    /// ```
    fn prepared_session(
        &self,
        ...
        root: Option<PathBuf>,
        task_workspace: Option<tempfile::TempDir>,
    ) -> PreparedTaskSession {
        PreparedTaskSession {
            ...
            root,
            _task_workspace: task_workspace,
        }
    }
