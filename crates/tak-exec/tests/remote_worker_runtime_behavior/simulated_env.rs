...
#[tokio::test]
async fn remote_worker_simulated_dockerfile_runtime_does_not_read_build_context() {
    let _env_lock = env_lock();
    let mut env_guard = EnvGuard::default();
    let temp = tempfile::tempdir().expect("tempdir");
    configure_fake_docker_env(temp.path(), &mut env_guard);
    ...
    fs::create_dir_all(&workspace_root).expect("create workspace");
    let spec = worker_spec(
        "remote_runtime_simulated_dockerfile",
        vec![shell_step(
            "printf '%s,%s,%s' \"$TAK_REMOTE_RUNTIME\" \"$TAK_REMOTE_ENGINE\" \"$TAK_RUNTIME_SOURCE\" > runtime.txt",
        )],
        None,
        Some(RemoteRuntimeSpec::Containerized {
