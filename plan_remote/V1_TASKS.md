...
- [x] `BDD` Real-world chain: `tak` running inside a container reaches remote `takd` over Tor transport and remote `takd` executes the task inside a real container, then returns synced output to caller. Evidence: `e2e_containerized_tak_client_runs_remote_tor_container_and_returns_outputs`.
- [x] `BDD` Recursive self-hosted remote smoke: containerized `tak` client reaches containerized remote `takd` via Tor transport and remote nested container runs selected Tak tests without recursive re-entry. Evidence: `e2e_remote_tor_recursive_remote_smoke_tests_run_in_nested_container`.
- [x] `BDD` Recursive self-hosted artifacts: remote nested container builds `tak` + `takd` binaries and syncs them back as outputs. Evidence: `e2e_remote_tor_recursive_builds_tak_binaries_and_syncs_them_back`.
